# ============================================================
# STRONG DEFINITE MISROUTES V2 — FULL RESULTS, >= 1 KM
#
# V2 additions:
...
strong_misroute_calls_v2 = calls[
...
if strong_misroute_calls_v2.empty:
    ...
    strong_misroute_pairs_v2["GEOMETRY_EVIDENCE"] = (
        f"Every call is definite misroute and >= "
    ...
    threshold_tag = f"over_{int(MIN_ROUTED_DISTANCE_M)}m"
    pair_path = OUTPUT_DIR / f"strong_misroute_pairs_{threshold_tag}_v2.csv"
    call_path = OUTPUT_DIR / f"strong_misroute_calls_{threshold_tag}_v2.csv"

    strong_misroute_pairs_v2.to_csv(pair_path, index=False)
    strong_misroute_calls_v2.to_csv(call_path, index=False)

    display(Markdown(
        f"## V2 strong PSAP pairs: routed distance >= "
    ...
    display(Markdown("## V2 individual calls with call and PSAP coordinates"))
    with pd.option_context(
    ...
    print(f"Saved V2 pair report: {pair_path}")
    print(f"Saved V2 call report: {call_path}")
